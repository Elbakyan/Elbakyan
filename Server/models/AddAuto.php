...
class AddAuto
{
    public static function Add_User_Auto($data,$file){
        global $mysql;
            ...
            if ($file['name'] == ''){
                $img_name_for_db = 'https://elbakyan.am/Server/img/default.png';
            }else{
            ...
            $res = $mysql->query("INSERT INTO `user_auto` (`id`, `user_id`, `mark`, `model`, `color`, `year`, `engine_power`, `auto_number`, `vin`, `img`,`type`, `data`) 
                                        VALUES (NULL, '$data[user_id]', '$data[mark]', '$data[color]', '$data[color]', '$data[year]', '$data[engine]', '$data[auto_number]', '$data[vin]', '$img_name_for_db','.$tmp_img_name[1]', CURRENT_TIMESTAMP);");
            if ($res){
